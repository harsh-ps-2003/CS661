...
# --- Production-ready function for metric vs. time bar plot ---
def plot_metrics_vs_time_bar(df, output_file, colors=None):
    """
    Plots all metrics present in the DataFrame (except the first column, which is the time axis) vs. time as a bar chart and saves as an HTML file.
    Args:
    ...
    Usage:
        ...
        # plot_metrics_vs_time_bar(df, 'output.html')
    """
    ...
    fig.update_layout(
        ...
        title='',
        autosize=True,
        barmode='group'  # Groups bars side by side for multiple metrics
    )
